...
public class ImageUploadUtil {
     ...
     * @Title upload
     */
    public static String uploadOld(HttpServletRequest request, String DirectoryName) throws IllegalStateException,
            IOException {
        // 创建一个通用的多部分解析器
...
//                        String t3=request.getServletContext().getRealPath("/");
...
//                        System.out.println("t:-------------"+t3);
                        ...
                        realPathDirectory.setExecutable(true);//设置可执行权限
                        realPathDirectory.setReadable(true);//设置可读权限
                        realPathDirectory.setWritable(true);//设置可写权限
                    }
                }
                // 记录上传该文件后的时间
            ...
            while (iter.hasNext()) {
                ...
                if (file != null) {
                    ...
                    if (myFileName.trim() != "") {
                        ...
                        fileName = FileUtil.uploadImage(request,file, DirectoryName);
...
//    					realPathDirectory.setWritable(true);//设置可写权限
    ...
    public static void ckeditor(HttpServletRequest request, HttpServletResponse response, String DirectoryName)
            throws IOException {
        DirectoryName = DirectoryName + "/" + DateUtil.thisDayBeginNew();
        String fileName = upload(request, DirectoryName);
        // 结合ckeditor功能
        // imageContextPath为图片在服务器地址，如upload/123.jpg,非绝对路径
        String uri = request.getRequestURI();
        String PathWithOut = "/" + DirectoryName + "/" + fileName;
        String PathWith = Constants.SUBJECT_PATH + "/" + fileName;
        String imageContextPath = "/" + DirectoryName + "/" + fileName;
        System.out.println("imageContextPath:" + uri);
        System.out.println("imageContextPath:" + imageContextPath);
        System.out.println("imageContextPath:" + PathWith);
        response.setContentType("text/html;charset=UTF-8");
        String callback = request.getParameter("CKEditorFuncNum");
        PrintWriter out = response.getWriter();
        out.println("<script type=\"text/javascript\">");
        out.println("window.parent.CKEDITOR.tools.callFunction(" + callback + ",'" + PathWith + "',''" + ")");
        out.println("</script>");
        out.flush();
        out.close();
    }
